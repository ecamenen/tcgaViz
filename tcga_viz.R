...
#geneX??
#OR import Gene X expression from xena
#GeneX <- read.delim("xenaDownload.tsv")

#Merge datasets + remove sample with NA
Dataset <- merge(TCGA_pop, Gene)
...
cutoff <- mean(Dataset$geneX)
#number of patients in each category should be indicated in the figure

cutted <- mutate(Dataset,high=geneX>cutoff)

#Plot sous type cell selon niveau d'expression du gène + stats
cutted_melt <- cutted
cutted_melt <- subset(cutted, select=-c(geneX))
cutted_melt <- cutted_melt[-c(1:2)]
cutted_melt  <- melt(cutted_melt)
colnames(cutted_melt)[2] <- "cell_type"
cutted_melt$value<-cutted_melt$value+1e-5
sub_cutted_melt <- subset(cutted_melt, cell_type==cell_type)
pop <- ggviolin(sub_cutted_melt,x="high", y="value", facet.by = "cell_type", outlier.shape = NA, 
                 color = "cell_type")+
  scale_x_discrete(name=" level", labels=c("FALSE"="Low","TRUE"="High")) + 
  scale_y_log10()
